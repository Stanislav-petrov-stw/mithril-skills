...
export function App() {
  ...
  // Live mode (VITE_ONBOARD_API set): the backend runs the real agent-wallet
  // registration and returns a real credentialId. Otherwise, Privy login.
  async function start() {
    if (!ONBOARD_API) { login(); return; }
    setStep("provisioning");
    try {
      const r = await fetch(ONBOARD_API, { method: "POST", headers: { "content-type": "application/json" }, body: "{}" });
      const d = await r.json();
      if (!d.credentialId) throw new Error("no cred");
      setCred(d.credentialId);
      setLive(true);
    } catch {
      setDemo(true);
      await wait(2200);
      ...
      <div className="card">
        {step === "signin" && (
          <div className="scr">
            ...
            <button className="btn google" onClick={start}>
        ...
        {step === "ready" && (
          <div className="scr">
            ...
            {live ? <div style={{ marginTop: 14, textAlign: "center", fontSize: 11, color: "#34d399", fontWeight: 600 }}>● live — real agent registered on Hyperliquid</div> : demo && <div className="badge-demo">demo mode — wire VITE_ONBOARD_API for the live flow</div>}
          </div>
        )}
